...
run_did_block <- function(dt, level, outcomes, treatments, k = 5,
  ...
  for (y in outcomes) {
    ...
    for (tr in treatments) {
      ...
      # Pull just treatment + prev control rows if present
      want <- ct %>% filter(term %in% c(tr, y_prev))
      
      # Keep treatment row as primary
      tr_row <- want %>% filter(term == tr)
      prev_row <- want %>% filter(term == y_prev)
...
res_bcy <- run_did_block(
  ...
  cluster = "loan_cert"   # alternative: "county"
)
